...
export const HorizonHero = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (refs.nebula) {
        ...
        if (refs.nebula.material instanceof Array) {
  ...
  useEffect(() => {
    ...
    const elements = [menuRef.current, titleRef.current, subtitleRef.current, scrollProgressRef.current].filter(Boolean);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      refs.mountains.forEach((mountain, i) => {
        ...
        if (progress < 0.7) {
          mountain.position.z = refs.locations?.[i] || location;
        }
      });
  ...
  return (
    <div ref={containerRef} className="relative w-full h-screen overflow-hidden">
      ...
      {/* Side menu */}
      <div ref={menuRef} className="fixed top-8 left-8 z-10 flex items-center gap-4" style={{ visibility: 'hidden' }}>
        <div className="flex flex-col gap-1.5 cursor-pointer">
          <span className="block w-6 h-0.5 bg-white"></span>
          <span className="block w-6 h-0.5 bg-white"></span>
          <span className="block w-6 h-0.5 bg-white"></span>
        </div>
        <div className="text-white text-sm font-mono tracking-widest -rotate-90 origin-left">SPACE</div>
      </div>

      {/* Main content */}
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 z-10">
        <h1 ref={titleRef} className="text-8xl md:text-9xl font-bold text-white mb-8 overflow-hidden">
          <span className="inline-block">
            {['H', 'O', 'R', 'I', 'Z', 'O', 'N'].map((char, i) => (
              <span key={i} className="title-char inline-block">{char}</span>
            ))}
          </span>
        </h1>
        
        <div ref={subtitleRef} className="text-xl md:text-2xl text-white/80 mb-16 space-y-2">
          <p className="subtitle-line opacity-0">
      ...
      <div ref={scrollProgressRef} className="fixed bottom-8 left-8 z-10 text-white text-sm font-mono flex items-center gap-4 opacity-0" style={{ visibility: 'hidden' }}>
        <div>SCROLL</div>
      ...
      <div className="relative z-0">
        {[...Array(2)].map((_, i) => {
          ...
          return (
            <section key={i} className="h-screen flex flex-col items-center justify-center px-4">
